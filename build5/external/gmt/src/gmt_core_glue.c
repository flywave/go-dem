...
/* gmt_core_glue.c populates the external array of this shared lib with
 ...
 *
 *   int core_module_show_all    (void *API);
 *   int core_module_list_all    (void *API);
 *   int core_module_classic_all (void *API);
 *
 ...
 *
 *   const char *core_module_keys  (void *API, char *candidate);
 *   const char *core_module_group (void *API, char *candidate);
 *
...
/* Sorted array with information for all GMT _core_ modules */

static struct GMT_MODULEINFO modules[] = {
#include "gmt_core_moduleinfo.h"
...
EXTERN_MSC int core_module_show_all (void *API) {
	return (GMT_Show_ModuleInfo (API, modules, "GMT core: The main modules of the Generic Mapping Tools", GMT_MODULE_HELP));
}

/* Produce single list on stdout of all shared module names for gmt --show-modules */
EXTERN_MSC int core_module_list_all (void *API) {
...
EXTERN_MSC int core_module_classic_all (void *API) {
	return (GMT_Show_ModuleInfo (API, modules, NULL, GMT_MODULE_SHOW_CLASSIC));
}

/* Lookup module id by name, return option keys pointer (for external API developers) */
EXTERN_MSC const char *core_module_keys (void *API, char *candidate) {
...
EXTERN_MSC const char *core_module_group (void *API, char *candidate) {
